...
export default function Kasir() {
  ...
  // USB barcode scanner detection (fast keystrokes ending with Enter)
  useEffect(() => {
  ...
  async function handlePayment() {
    ...
    try {
      ...
      setCart([]); setCashInput(''); setPayModal(false)
      loadProducts()
    } catch (err) {
  ...
  return (
    <div className="h-screen flex flex-col bg-neutral-50 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="bg-white border-b border-neutral-200 px-4 py-3 flex gap-2 flex-shrink-0">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Cari barang... (tekan F1)"
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-neutral-400 gap-2">
                ...
                <span className="text-sm">Tidak ada produk di kategori ini</span>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {filteredProducts.map(product => (
                  <button key={product.id}
                    ...
                    className={`bg-white border rounded-xl p-3 text-center transition-all hover:shadow-sm active:scale-95 ${product.stock === 0 ? 'opacity-40 cursor-not-allowed border-neutral-100' : 'border-neutral-200 hover:border-neutral-300 cursor-pointer'}`}>
                    {product.photo_url ? (
                      <img src={product.photo_url} alt={product.name} className="w-full aspect-square object-cover rounded-lg mb-2" />
                    ) : (
                      <div className="w-full aspect-square bg-neutral-50 rounded-lg mb-2 flex items-center justify-center text-3xl">
                    ...
                    <div className={`text-xs mt-1 ${product.stock <= 5 ? 'text-orange-500' : 'text-neutral-400'}`}>Stok: {product.stock}</div>
                  </button>
                ))}
              </div>
            )}
        ...
        {/* RIGHT — CART */}
        <div className="w-72 lg:w-80 bg-white border-l border-neutral-200 flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-neutral-300 gap-2 p-8">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/>
                </svg>
                <span className="text-xs text-center">Klik produk, scan kamera, atau gunakan scanner USB</span>
              </div>
            ) : (
      ...
      {payModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xs p-6 shadow-xl">
            ...
            <div className="bg-neutral-50 rounded-xl p-3 mb-4 space-y-1 max-h-32 overflow-y-auto">
              {cart.map(item => (
